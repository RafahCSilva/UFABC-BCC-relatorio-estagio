...
\newcommand{\defRelatorio}{RELATÓRIO DO ESTÁGIO SUPERVISIONADO II} %%ESTÁGIO SUPERVISIONADO
...
Clientes são de grande importância para qualquer empresa, conhece-los é crucial para agregar mais valor ao seus produtos e serviços que são comercializados, para assim criar uma relação duradoura que melhor atenda os objetivos de ambos os lados. A Cadaris é uma agência de comunicação e tratar bem o Cliente é essencial, e todos artifícios para melhor compreende-los se faz necessário. 

Na Cadaris seus Clientes são organizados numa forma de árvore hierárquica. Na qual o primeiro nível é o conjunto de Empresas, no segundo nível os Departamentos da Empresa e no terceiro, e último, nível trata-se dos Centros de Custos daquele Departamento. E dado esta estrutura complexa e por anteriormente a empresa utilizar planilhas para armazenar cadastros, criou-se a conveniência de atribuir códigos para indicar a hierarquia dos itens, como no exemplo a seguir:
...
\begin{singlespace}
	\begin{description}
		...
		\begin{description}
			...
			\begin{description}
				\item[$\bullet$~~~10.03.01] Centro de Custo A1-a
				\item[$\bullet$~~~10.03.02] Centro de Custo A1-b
				\item[$\bullet$~~~10.03.03] Centro de Custo A1-c
			\end{description}
...
Assim no Cadastro dos Clientes devem preservar esta estrutura e também os códigos atribuídos a cada item, afim de manter a conveniência criada pelos Funcionários. Então em cada item é cadastrado um nome e seu código a ser exibido. 

Na Empresa do Cliente é cadastrados todas suas unidades, e nestas Unidades há os seguintes dados: Nome Fantasia, Razão Social, CNPJ, Inscrição Estadual, Inscrição Municipal, Condição de Pagamento (utilizada no módulo Comercial), Informação Bancária e Endereço Completo.
...
E dentro de cada Departamento é cadastrado o Centro de Custo, sem dados adicionais. Deste modo, possibilita cada \textit{Job} da Cadaris estar associado a um Centro de Custo do seu Cliente, e com os dados herdado dos itens superiores ajudarão a formar a Proposta Comercial, que será entregue para o Cliente e executada pela Cadaris, como será relatado na seção~\hyperref[sec:2.3.2]{2.3.2 - Comercial}.
...
\subsubsubsection{CENTROS DE CUSTO}
